    ...
    public partial class Form1 : Form
    {
        ...
        public void StartDownload()
        {
            ...
            for (int i = 0; i < games.Count; i++)
            {
                ...
                var lastGamePath = PathGames + alias + "\\roms\\" + games[i].Name.Replace(' ','_').Replace(':',' ') + ".7z";
               /* var parseSet = new UrlParse.UrlSettings(games[i].Url);
                ...
                foreach (var prefix in bset.Prefix)
                {
                    ...
                    if (System.IO.File.Exists(PathGames + alias + "\\pics\\"+ games[i].Name.Replace(' ', '_').Replace(':', ' ') + "0.jpg"))
                        System.IO.File.Delete(PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + "0.jpg");
                    System.IO.File.WriteAllBytes(PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + "0.jpg", buffer_pic);
                    InsertUrlGame(games[i], PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + "0.jpg", 2);
                    ...
                    foreach (var img in imgresult.Where(dd => dd != games[i].ImgUrl).ToArray())
                    {
                        ...
                        if (System.IO.File.Exists(PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + pi.ToString()+".jpg"))
                            System.IO.File.Delete(PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + pi.ToString()+".jpg");
                        System.IO.File.WriteAllBytes(PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + pi.ToString() + ".jpg", buffer_pic);
                        InsertUrlGame(games[i], PathGames + alias + "\\pics\\" + games[i].Name.Replace(' ', '_').Replace(':', ' ') + pi.ToString() + ".jpg", 3);
                    }
